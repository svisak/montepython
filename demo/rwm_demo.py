...
class MultivariateGaussian(montepython.SimpleBayes):
    ...
    def evaluate(self, position):
        ...
        You must set posterior value here.
        Preferably with the set method as shown.
        How you get hold of value is up to you ...
        '''
...
# Create RWM object
rwm = montepython.RWM(bayes, startpos, stepsize=stepsize)
...
filename = 'rwm.h5' # Optional. Defaults to a timestamp. I recommend using the default.
...
rwm.batched_run_for(t_limit, n_batches, unit=unit, path=path, filename=filename, **additional_info)
print(f'Finished {n_batches} batches.')
print(f'Number of samples: {len(rwm.chain())}')

# See the docstring/code/README for more options/information. 

# Alternatively, you can use run() or run_for() and call rwm.to_disk() manually.
# Or you can get the chain itself with rwm.chain() and save it yourself however you like.
